                            ...
                            <div class="mb-3">
                                ...
                                <select name="posisi" class="form-select" required>
                                    ...
                                    @foreach ($posisiOptions as $value => $label)
                                        <option value="{{ $value }}" {{ old('posisi') === $value ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                            </div>
                                ...
                                <tbody>
                                    @forelse ($topLeaders as $topLeader)
                                        <tr>
                                            ...
                                            <td><span class="badge text-bg-light text-dark border">{{ $topLeader->posisi }}</span></td>
                                            ...
                                            <td class="text-end">
                                                <div class="btn-group btn-group-sm">
                                                    <button type="button" class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-target="#editTopLeader{{ $topLeader->id }}">
                                                        <i class="bi bi-pencil"></i> Edit
                                                    </button>
                                                    <form action="{{ route('top-leader.destroy', $topLeader) }}" method="POST" class="d-inline" onsubmit="return confirm('Hapus Top Leader {{ $topLeader->nama }}?')">
                                                        @csrf @method('DELETE')
                                        ...
                                        <div class="modal fade" id="editTopLeader{{ $topLeader->id }}" tabindex="-1">
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <form action="{{ route('top-leader.update', $topLeader) }}" method="POST">
                                                        @csrf @method('PUT')
                                                        <div class="modal-header">
                                                            <h5 class="modal-title">Edit Top Leader: {{ $topLeader->nama }}</h5>
                                                            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                                        </div>
                                                        <div class="modal-body">
                                                            <div class="mb-3">
                                                                <label class="form-label">Nama</label>
                                                                <input type="text" name="nama" class="form-control" value="{{ $topLeader->nama }}" required>
                                                            </div>
                                                            <div class="mb-3">
                                                                <label class="form-label">Posisi</label>
                                                                <select name="posisi" class="form-select" required>
                                                                    @foreach ($posisiOptions as $value => $label)
                                                                        <option value="{{ $value }}" {{ $topLeader->posisi === $value ? 'selected' : '' }}>{{ $label }}</option>
                                                                    @endforeach
                                                                </select>
                                                            </div>
                                                            <div class="form-check form-switch">
    <input type="hidden" name="is_active" value="0">
    <input class="form-check-input" type="checkbox" name="is_active" value="1" id="active{{ $topLeader->id }}" {{ $topLeader->is_active ? 'checked' : '' }}>
    <label class="form-check-label" for="active{{ $topLeader->id }}">Aktif</label>
</div>
                                                        </div>
